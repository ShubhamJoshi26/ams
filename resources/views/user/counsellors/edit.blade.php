<div class="modal-body">
  ...
  <div class="text-center mb-6">
    <h4 class="mb-2">Edit Counsellor</h4>
  </div>

  <form id="addCounsellorManager" class="row g-6" action="{{ route('counsellors.update') }}" method="post">
  <div class="col-md-6">
    <label class="form-label" for="user_type">User Type</label>
    <select class="form-select form-control" id="user_type" name="user_type">
        <option value="">Choose</option>
        <option value="1" @if (isset($getData[0]['user_type']->user_type) && $getData[0]['user_type']->user_type == 1) selected @endif>Outsourced</option>
        <option value="0" @if (isset($getData[0]['user_type']->user_type) && $getData[0]['user_type']->user_type == 0) selected @endif>Inhouse</option>
    </select>
  </div>  
